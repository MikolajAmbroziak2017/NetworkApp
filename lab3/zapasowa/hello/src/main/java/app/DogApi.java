...
public class DogApi {
    ...
    public ResponseEntity<List<Dog>> getDog() { //pzzekazujemy liste do clienta
        ResponseEntity <List<Dog>> listResponseEntity = new ResponseEntity<List<Dog>>(petmenager.getDogList(), HttpStatus.OK); // obsługa błędu serweru
   ...
   return ResponseEntity.badRequest().build();
    ...
    public Dog  getDogById (@PathVariable("dog_id") Integer id) {
        if (id>petmenager.getDogList().size())
            return null;
        return petmenager.getDogById(id);
    }
